...
function addNewOrderToUI(orderData) {
  ...
  try {
    ...
    console.log(
      "[debug] Created order card:",
      orderCard,
      orderCard ? orderCard.outerHTML : orderCard
    );
...
function updateOrderInUI(updateData) {
  const orderElement = document.querySelector(
    `[data-order-id="${updateData.orderId}"]`
  );
...
function createOrderCardElement(orderData) {
  ...
  try {
    ...
    orderCard.setAttribute(
      "data-order-id",
      orderData.orderId || orderData.id || "unknown"
    );
    ...
    const totalPrice =
      typeof orderData.totalPrice === "number" && !isNaN(orderData.totalPrice)
        ? orderData.totalPrice
        : 0;
    const orderNote =
      typeof orderData.note === "string" && orderData.note.trim()
        ? orderData.note
        : null;
    ...
    orderCard.innerHTML = `
            ...
            ${
              orderNote
                ? `<div class="order-info"><span class="order-note"><i class="fa fa-sticky-note"></i> <span>${orderNote}</span></span></div>`
                : ""
            }
            <div class="order-total"><span class="total-price">Tổng: ${formatPrice(
              totalPrice
            )} VNĐ</span></div>
            <a href="/pizzario/kitchen/order/${
              orderData.orderId || orderData.id || "unknown"
            }" class="order-detail-btn">Xem chi tiết</a>
        `;
  } catch (error) {
...
function updateCompletedItems(orderElement, completed, total) {
  const completedSpan = orderElement.querySelector(
    ".order-progress span:last-child"
  );
  if (completedSpan) {
...
function upsertOrderCard(orderData) {
  ...
  let orderCard = document.querySelector(
  ...
  if (orderCard) {
    ...
    if (tableEl)
      tableEl.innerHTML = `<i class="fa fa-map-marker-alt"></i> ${
        orderData.tableName || orderData.type || "N/A"
      }`;
    ...
    if (totalPriceEl)
      totalPriceEl.textContent = `Tổng: ${formatPrice(totalPrice)} VNĐ`;
    const orderNote =
      typeof orderData.note === "string" && orderData.note.trim()
        ? orderData.note
        : null;
    const noteWrapper = orderCard.querySelector(
      ".order-info .order-note span:last-child"
    );
